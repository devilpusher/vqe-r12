#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Step 10b: build a Step-4b-like Be ECG-NO R12 bridge file."""
...
def parse_args():
    ...
    p.add_argument("--rdm-inp", default="step10a_be_sp_s012_p0_rdm_export.npz")
    ...
    p.add_argument("--basis-name", default="beecgnosp")
...
def psi4_basis_string(name: str, channels: List[str], exponents: Dict[str, np.ndarray]) -> str:
    lines = [f"assign {name}", f"[{name}]", "spherical", "****", "Be 0"]
    for ch in channels:
...
def compute_parent_integrals_r12_optional(wfn, mints, corr, r12_only: bool) -> Dict[str, np.ndarray]:
    ...
    return {
        ...
        "h": asarray_psi4(mints.ao_kinetic()) + asarray_psi4(mints.ao_potential()),
...
def main():
    ...
    args.psi4_output = args.psi4_output or f"psi4_step10b_be_{tag}.out"
    args.out = args.out or f"step10b_be_{tag}_step4b_like.npz"
    args.summary = args.summary or f"step10b_be_{tag}_step4b_like_summary.txt"
    args.json = args.json or f"step10b_be_{tag}_step4b_like.json"
    ...
    with pushd(be_dir):
        ...
        mol_pyscf, exponents, ao_index = be.build_mol_and_layout(ns, channels)
        S_pyscf = mol_pyscf.intor("int1e_ovlp")
        C_obs, labels = be.build_ecg_no_subspace(mats, ns, mol_pyscf, S_pyscf, ao_index, channels)
    ...
    if nao != mol_pyscf.nao or direct_overlap_error > 1e-8 or cobs_orth_psi4 > 1e-8:
        raise RuntimeError(f"PySCF/Psi4 mismatch: nao {mol_pyscf.nao}/{nao}, S {direct_overlap_error:.3e}, C {cobs_orth_psi4:.3e}")
    ...
    check_finite_many({"h_obs": h_obs, "eri_obs": eri_obs, "h_ri": h_ri, "eri_ri": eri_ri, "f12_ri": f12_ri, "dm1_obs": dm1_obs, "dm2_obs": dm2_obs})
    ...
    if abs(checks["trace_dm1_error"]) > 1e-10 or abs(checks["trace_dm2_error"]) > 1e-10:
        failures.append("rdm_trace")
    ...
    metadata = {
        ...
        "enuc": enuc,
        "corr_info": corr_info,
        "rdm_source": "Step10a Be selected ECG-NO-FCI RDM",
        "energies": {"E_obs_fci": float(input_energy), "E_obs_rdm": float(E_obs_rdm), "E_ri_embedded_rdm": float(E_ri_rdm), "E1_obs": float(E1_obs), "E2_obs": float(E2_obs), "E1_ri": float(E1_ri), "E2_ri": float(E2_ri)},
        "checks": checks,
        "passed": not failures,
        "failures": failures,
    ...
    lines = [
        "=" * 96,
        "Step 10b | Be selected-OBS Step-4b-like R12 bridge",
        "=" * 96,
        f"be_dir       = {be_dir}",
        ...
        f"E_input_Be_FCI                   = {input_energy: .14f} Eh",
