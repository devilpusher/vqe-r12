#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Step 9e: Li pair/channel audit for selected-space R12 corrections."""
...
def parse_args():
    ...
    p.add_argument("--prefix", default="step9e_li_pair_channel_audit_fitN7")
...
def paths(case: str, fitn: int) -> Dict[str, str]:
    return {
        "rdm_npz": f"step9a_li_{case}_rdm_export.npz",
        "bridge_npz": f"step9b_li_{case}_fitN{fitn}_step4b_like.npz",
        "corr_json": f"step9c_li_{case}_fitN{fitn}_sf2r12_correction.json",
    }
...
def load_case(case: str, fitn: int) -> Dict[str, Any]:
    ...
    if missing:
        raise FileNotFoundError(f"Missing prerequisite files for {case}: {missing}. Run step9d first.")
...
def write_outputs(args, channel_rows: List[Dict[str, Any]], spin_rows: List[Dict[str, Any]], label_rows: List[Dict[str, Any]], summaries: List[Dict[str, Any]]) -> None:
    ...
    with open(args.out_json, "w", encoding="utf-8") as f:
        json.dump({"step": "9e", "fitN": args.fitN, "summaries": summaries, "channel_rows": channel_rows}, f, indent=2)
    ...
    lines = [
        "=" * 118,
        "Step 9e | Li pair population and R12 V-channel audit",
        "=" * 118,
        f"fitN = {args.fitN}",
        "",
        f"{'case':<14s} {'V_total/mEh':>12s} {'V_s-s':>12s} {'V_s-p':>12s} {'V_p-p':>12s} {'ab_frac':>9s} {'ss_pair_frac':>12s}",
        "-" * 118,
    ]
    for s in summaries:
        lines.append(
            f"{s['case']:<14s} {s['V_total_mEh']:12.6f} {s.get('V_s-s_mEh', 0.0):12.6f} "
    ...
    lines.extend(
        [
            "",
            "[Reading]",
            "V_s-s should shrink strongly when the third s radial NO is included if Li R12 is mostly radial/core cusp recovery.",
            "Pair fractions are spin-orbital populations; V rows are spin-free contraction contributions.",
            "",
            "[Saved]",
            f"  {args.out_json}",
